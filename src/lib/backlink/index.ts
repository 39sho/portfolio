import { parse, type ComarkTree, type ComarkNode } from "comark";
 ...
 */
export async function getBacklinks(posts: Post[]): Promise<Map<string, string[]>> {
    const validCollections = new Set<string>(posts.map((post) => post.collection));
